...
void PWM_init(void)
{
	...
	// Force a software UPG after writing odd CREP so update events are guaranteed
	// to land on underflow (TRM §15.1.4: "If an update event is generated by
	// software after writing an odd number to CREP, the update events will be
	// generated on the underflow.").
	timer_event_software_generate(TIMER_BLDC, TIMER_EVENT_SRC_UPG);

	// Export TIMER0's update event as TRGO. Used by the ADC-trigger timer
	// (TIMER2, set up in ADC_Trigger_Timer_init) as the synchronisation
	// source — every PWM valley restarts TIMER2, which then fires its own
	// compare-match at a programmable offset to drive the ADC regular
	// group in hardware. GD32F130 can't route T0 TRGO directly to the
	// regular group (ETSRC offers T0_CH0/CH1/CH2 only, which are moving
	// PWM outputs), so TIMER2 acts as the bridge.
	timer_master_output_trigger_source_select(TIMER_BLDC, TIMER_TRI_OUT_SRC_UPDATE);
